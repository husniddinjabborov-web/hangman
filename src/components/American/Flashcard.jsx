...
function Flashcard({ flashcardGroups }) {
  ...
  useEffect(() => {
    const flashcards = JSON.parse(localStorage.getItem('flashcard-american')) || flashcardGroups;
    setLocalFlashcards(flashcards);

  }, [flashcardGroups]);

  const updateLocalStorage = (updatedFlashcards) => {
    localStorage.setItem('flashcard-american', JSON.stringify(updatedFlashcards));
    setLocalFlashcards(updatedFlashcards);
  };

  const updateLearnedGroups = (groupIndex) => {
    const learnedGroups = JSON.parse(localStorage.getItem('learned-american')) || [];
    ...
    if (!learnedGroups.some((g) => JSON.stringify(g) === JSON.stringify(firstCardArray))) {
      ...
      localStorage.setItem('learned-american', JSON.stringify(learnedGroups));
    }
  };

  useEffect(() => {
    const savedTimers = JSON.parse(localStorage.getItem("timers-american")) || {};
    const updatedTimers = {};
    ...
    localStorage.setItem("timers-american", JSON.stringify(savedTimers));
  ...
  const handleFlip = (groupIndex, cardIndex, inputText) => {
    ...
    localStorage.setItem("timers-american", JSON.stringify({ ...JSON.parse(localStorage.getItem("timers-american") || "{}"), [cardKey]: startTime }));
  };
